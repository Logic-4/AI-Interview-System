  ...
  return (
    <div className="max-w-4xl mx-auto py-6 space-y-6 animate-in fade-in duration-500 text-black dark:text-white-dark">
      <div className="space-y-1">
        <h1 className="text-2xl font-bold tracking-tight text-text-primary dark:text-white">Leaderboard</h1>
        <p className="text-sm text-text-muted font-semibold">See how you rank among other interviewees</p>
      </div>

      {/* Your Stats */}
      <Card hoverEffect={false} className="p-6 border border-primary/20 bg-primary/5 relative overflow-hidden">
        <div className="flex items-center gap-4">
          <div className="w-14 h-14 rounded-md overflow-hidden border-2 border-primary/30 bg-white-light dark:bg-[#1b2e4b] flex items-center justify-center">
            {avatarSrc ? (
              <img src={avatarSrc} alt={user?.name ?? "You"} className="w-full h-full object-cover" />
            ) : (
              <User className="w-5 h-5 text-text-muted" />
            )}
          </div>
          <div className="flex-1">
            <p className="text-sm font-bold text-text-primary dark:text-white">{user?.name ?? "You"}</p>
            <p className="text-xs text-text-muted font-semibold">{user?.targetRole || "Interview Candidate"}</p>
          </div>
        ...
        <div className="grid grid-cols-3 gap-4 mt-5">
          {[
            ...
            const Icon = s.icon;
            return (
              <div key={s.label} className="text-center p-3 rounded-md bg-white/50 dark:bg-[#1a2941]/30 border border-white-light dark:border-[#1b2e4b]">
                <Icon className="w-4 h-4 text-primary mx-auto mb-1.5" />
                <p className="text-sm font-bold text-text-primary dark:text-white">{s.value}</p>
      ...
      <Card hoverEffect={false} className="p-6 border border-white-light dark:border-[#1b2e4b] bg-white dark:bg-black">
        ...
        <div className="space-y-3">
          {[
            ...
            { tier: "Silver", range: "70-79%", icon: Medal, color: "text-text-muted", bg: "bg-foreground/5", border: "border-[#1b2e4b]" },
            { tier: "Bronze", range: "60-69%", icon: Star, color: "text-warning", bg: "bg-warning/10", border: "border-warning/20" },
            { tier: "Beginner", range: "0-59%", icon: TrendingUp, color: "text-text-muted", bg: "bg-foreground/5", border: "border-[#1b2e4b]" },
          ].map((t, i) => {
            const Icon = t.icon;
            ...
            return (
              <motion.div
                ...
                className={cn(
                  ...
                  isCurrentTier ? `${t.bg} ${t.border} ring-2 ring-primary/20` : "border-white-light dark:border-[#1b2e4b]/40 bg-white-light/30 dark:bg-[#1a2941]/30"
                )}
              >
                <div className={cn("w-10 h-10 rounded-md flex items-center justify-center", t.bg)}>
                  <Icon className={cn("w-5 h-5", t.color)} />
                </div>
                <div className="flex-1">
                  <p className="text-xs font-bold text-text-primary dark:text-white">{t.tier}</p>
                  <p className="text-[10px] text-text-muted font-semibold">{t.range}</p>
                </div>
                {isCurrentTier && (
      ...
      <Card hoverEffect={false} className="p-8 border border-white-light dark:border-[#1b2e4b] bg-white dark:bg-black text-center">
        ...
        <h3 className="text-sm font-bold text-text-primary dark:text-white mb-1">Community Leaderboard Coming Soon</h3>
