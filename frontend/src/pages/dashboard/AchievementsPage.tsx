  ...
  return (
    <div className="max-w-4xl mx-auto py-6 space-y-6 animate-in fade-in duration-500 text-black dark:text-white-dark">
      <div className="space-y-1">
        <h1 className="text-2xl font-bold tracking-tight text-text-primary dark:text-white">Achievements</h1>
      ...
      <Card hoverEffect={false} className="p-6 border border-white-light dark:border-[#1b2e4b] bg-white dark:bg-black">
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1">
            <p className="text-sm font-bold text-text-primary dark:text-white">{unlocked.length} of {ACHIEVEMENTS.length} Achievements</p>
            <div className="h-2 w-full bg-white-light dark:bg-[#1b2e4b] rounded-full overflow-hidden mt-2">
              <motion.div
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {unlocked.map((a, i) => {
              ...
              return (
                ...
                >
                  <Card hoverEffect={false} className="p-4 border border-white-light dark:border-[#1b2e4b] bg-white dark:bg-black">
                    <div className="flex items-center gap-3">
                      <div className={cn("w-10 h-10 rounded-md flex items-center justify-center", a.bgColor)}>
                        <Icon className={cn("w-5 h-5", a.color)} />
                      </div>
                      <div>
                        <p className="text-xs font-bold text-text-primary dark:text-white">{a.title}</p>
                        <p className="text-[10px] text-text-muted font-semibold mt-0.5">{a.description}</p>
                      </div>
                    </div>
                  </Card>
                </motion.div>
      ...
      {locked.length > 0 && (
        <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {locked.map((a) => (
              <Card key={a.id} hoverEffect={false} className="p-4 border border-white-light dark:border-[#1b2e4b] bg-white dark:bg-black opacity-50">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-md bg-white-light/30 dark:bg-[#1b2e4b] flex items-center justify-center">
                    <Lock className="w-4 h-4 text-text-muted" />
                  </div>
                  <div>
                    <p className="text-xs font-bold text-text-muted">{a.title}</p>
                    <p className="text-[10px] text-text-muted font-semibold mt-0.5">{a.description}</p>
                  </div>
                </div>
              </Card>
