...
const githubRedirect = (_req, res) => {
  const params = new URLSearchParams({
    client_id: process.env.GITHUB_CLIENT_ID,
    redirect_uri: `${process.env.API_BASE_URL || 'http://localhost:5000'}/api/v1/auth/github/callback`,
  ...
  try {
    ...
    const [profileRes, emailsRes] = await Promise.all([
      fetch('https://api.github.com/user', {
        headers: { Authorization: `Bearer ${tokenData.access_token}`, 'User-Agent': 'InterviewAI' },
      }),
    ...
    const primaryEmail = emails.find((e) => e.primary && e.verified)?.email || emails[0]?.email || profile.email;
    ...
    logger.error(`GitHub OAuth failed: ${error.message}`);
    res.redirect(`${CLIENT_URL}/login?error=${encodeURIComponent('GitHub sign-in failed.')}`);
  }
};
